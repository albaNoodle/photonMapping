...
public class PathTracer {
	...
	private static int N_PHOTONS= 10000;
	private static PhotonMapping pm = new PhotonMapping();
	...
	public static void main(String[] args) {
		...
		figures.add(new Plane(new Point(0, 0, 1), new Direction(0, 0, -1), Color.GRAY, 90, 0, 1, 0.0)); // fondo
//		figures.add(new Plane(new Point(0, 0, 1), new Direction(0, 0, 1), Color.YELLOW, 40, 0.5, 0, 0.0)); //
		...
		figures.add(new Sphere(new Point(-65, -25, 70), 13, Color.MAGENTA, 0.3, 0.0, 0));
...
//				new Direction(0, 1, 0), Color.ORANGE, 0.5, 0.0, 0.0, false));
		...
		Color finalColor;
		pm.sendPhotons( figures, light1, N_PHOTONS);
		// Bucle Pixels Image/
...
//		Point light = new Point(-35, 39.98, 45);
	...
	//			else {
				...
				else if (mC == 2 && !f.is_light) {		
					ld = luzDirectaEspecular(intersection, lightPoints.get(i), figures, f, tt, 0);
...
//			gA = gA +  lI.getGreen();
	...
	static Color luzIndirectaDifusa(Point intersect, Point light, List<Figure> figures, Figure fi, Ray ray,
			int bounds) {
		// new ray
		/******************************/
...
//				lD = new Color((int)255/(bounds+1), 0 ,(int)255/(bounds+1));
	...
	private static Color luzDirectaEspecular(Point intersect, Point light, List<Figure> figures, Figure fi, Ray ray,
			int bounds) {
		// new specular
		/******************************/
		...
		int mC = RussianRoulette.calculate(f);
		if (mC == 2 && !f.is_light) {
			lE = luzDirectaEspecular(intersection, light, figures, f, ray, 0);
		}
		else if(!f.is_light) lE = luzDirecta(intersection, light, figures, f,ray.getD(), POTENCIA);
		return lE;
	}
	static Color luzIndirectaEspecular(Point intersect, Point light, List<Figure> figures, Figure fi, Ray ray,
			int bounds) {
		// new specular
		/******************************/
		double i = Operator.dotProduct(ray.getD(), fi.getNormal(intersect));
